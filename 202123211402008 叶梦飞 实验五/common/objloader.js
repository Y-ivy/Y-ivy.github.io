(function (scope, undefined) {
        ...
        */
        ...
        for (var j = 0, eleLen = elements.length; j < eleLen; j++) {
          ...
          if (elements[j] in unpacked.hashindices) {
            ...
            if (x > unpacked.xmax) unpacked.xmax = x;
            if (x < unpacked.xmin) unpacked.xmin = x;
            var y = parseFloat(verts[(vertex[0] - 1) * 3 + 1]);
            if (y > unpacked.ymax) unpacked.ymax = y;
            if (y < unpacked.ymin) unpacked.ymin = y;
            var z = parseFloat(verts[(vertex[0] - 1) * 3 + 2]);
            if (z > unpacked.zmax) unpacked.zmax = z;
            if (z < unpacked.zmin) unpacked.zmin = z;
            /* ========== end of Lyii */
            ...
            if (vertNormals.length) {
              ...
              var localv1 = [
                +verts[(elements[0] - 1) * 3 + 0],
                +verts[(elements[0] - 1) * 3 + 1],
                +verts[(elements[0] - 1) * 3 + 2],
              ];
              var localv2 = [
                +verts[(elements[1] - 1) * 3 + 0],
                +verts[(elements[1] - 1) * 3 + 1],
                +verts[(elements[1] - 1) * 3 + 2],
              ];
              var localv3 = [
                +verts[(elements[2] - 1) * 3 + 0],
                +verts[(elements[2] - 1) * 3 + 1],
                +verts[(elements[2] - 1) * 3 + 2],
              ];
              ...
              var len =
                1.0 /
                Math.sqrt(crs[0] * crs[0] + crs[1] * crs[1] + crs[2] * crs[2]);
          ...
          (function (name) {
            return function (data, status) {
              if (status === 200) {
                ...
                error = true;
                console.error(
                  'An error has occurred and the mesh "' +
                    name +
              ...
              if (semaphore === 0) {
                if (error) {
                  ...
                  console.error(
                    "An error has occurred and one or meshes has not been " +
  ...
  OBJ.initMeshBuffers = function (gl, mesh) {
    mesh.normalBuffer = _buildBuffer(
      gl,
      gl.ARRAY_BUFFER,
      mesh.vertexNormals,
      3
